...
Every command ships as both a **Slash Command** (`/`) and a **Prefix Command** (`-`).

## 1. General

| Command | Description |
| --- | --- |
| `bug` | Opens a modal to report a bot bug to developers |
...
| `factcheck` | Factchecks someone's message |
...
| `vape` | Hit the vape |
| `vape flavor` | Sets or changes your virtual vape flavor |
| `vape hits` | Displays total recorded virtual vape hits |
| `vape steal` | Steal the vape |
| `ttys` | The bot begins to talk to itself until someone sends a message or after an hour |
...
- **Guild picker** — lists all guilds the bot is in, with per-guild deleted message and mod action counts
- **Deleted messages** — paginated view of deleted message content, author, channel, and who deleted it
- **Moderation actions** — paginated view of all mod actions (ban, kick, timeout, warn, etc.) with actor/target names and reasons

Authentication is handled via Discord OAuth2 (session cookie).
